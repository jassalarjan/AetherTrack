...
import { Plus, Users, CheckSquare, TrendingUp, Clock, FileSpreadsheet, FileText, AlertTriangle, Calendar, Filter, X, Download, Smartphone, X as CloseIcon, LayoutDashboard } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // Handler functions for stat card clicks
  const handleStatCardClick = (category, color = 'primary') => {
    let tasksToShow = [];
    let title = '';
    ...
    switch (category) {
      ...
      case 'myTasks':
        tasksToShow = allTasks.filter(task => 
          task.assigned_to?.some(assignee => assignee._id === user?.id || assignee === user?.id)
        );
        title = 'My Tasks';
        break;
      case 'inProgress':
        tasksToShow = allTasks.filter(task => task.status === 'in_progress');
        title = 'In Progress Tasks';
        break;
      case 'completed':
        tasksToShow = allTasks.filter(task => task.status === 'done');
      ...
      case 'overdue':
        tasksToShow = allTasks.filter(task => {
          ...
          return dueDate < today && task.status !== 'done' && task.status !== 'archived';
        });
        title = 'Overdue Tasks';
        break;
      default:
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          <PageHeader 
            ...
            subtitle="Here's an overview of your tasks and team performance"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            <StatCard
              ...
              color="primary"
              loading={loading}
              onClick={() => handleStatCardClick('total', 'primary')}
            />
            ...
            <StatCard
              ...
              color="success"
              loading={loading}
              onClick={() => handleStatCardClick('myTasks', 'success')}
            />
            ...
            <StatCard
              ...
              color="warning"
              loading={loading}
              onClick={() => handleStatCardClick('inProgress', 'warning')}
            />
            ...
            <StatCard
              ...
              color="purple"
              loading={loading}
              onClick={() => handleStatCardClick('completed', 'purple')}
            />
            ...
            <StatCard
              ...
              color="danger"
              loading={loading}
              onClick={() => handleStatCardClick('overdue', 'danger')}
            />
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="action-buttons">
              <Link
                to="/tasks"
                className="action-btn bg-gradient-purple text-white shadow-lg hover:shadow-xl"
              ...
              <Link
                to="/kanban"
                className="action-btn bg-gradient-to-r from-violet-500 to-violet-600 text-white shadow-lg hover:shadow-xl"
              >
              ...
              {['admin', 'hr'].includes(user?.role) && (
                <Link
                  to="/teams"
                  className="action-btn bg-gradient-to-r from-fuchsia-500 to-fuchsia-600 text-white shadow-lg hover:shadow-xl"
              ...
              <Link
                to="/analytics"
                className="action-btn bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg hover:shadow-xl"
              >
              ...
              {['admin', 'hr'].includes(user?.role) && (
                <>
                  <button
                    ...
                    className="action-btn bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg hover:shadow-xl"
                  ...
                  <button
                    ...
                    className="action-btn bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg hover:shadow-xl"
          ...
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
              ...
              <div className={`px-6 py-5 ${
                modalColor === 'primary' ? 'bg-gradient-to-r from-primary-500 to-primary-700' :
                modalColor === 'success' ? 'bg-gradient-to-r from-emerald-500 to-emerald-700' :
                modalColor === 'warning' ? 'bg-gradient-to-r from-amber-500 to-amber-700' :
                modalColor === 'danger' ? 'bg-gradient-to-r from-red-500 to-red-700' :
                modalColor === 'purple' ? 'bg-gradient-to-r from-purple-500 to-purple-700' :
                ...
                <div className="flex items-center justify-between">
                  <h3 className="text-2xl font-bold text-white flex items-center gap-3">
                    ...
                    <span className="text-sm font-normal bg-white/20 px-3 py-1 rounded-full">
                      {modalTasks.length} {modalTasks.length === 1 ? 'task' : 'tasks'}
                    </span>
                  </h3>
                  <button
                    onClick={() => setShowTaskModal(false)}
                  ...
                  <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    {modalTasks.map((task) => {
                      ...
                      return (
                        <div 
                          key={task._id} 
                          className={`p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-l-4 ${
                            isOverdue ? 'border-red-500 bg-red-50/50 dark:bg-red-900/10' : 
                            task.status === 'done' ? 'border-green-500' :
                            task.status === 'in_progress' ? 'border-purple-500' :
                            task.status === 'review' ? 'border-yellow-500' :
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {task.assigned_to && task.assigned_to.length > 0 && (
                                  <span className="text-xs flex items-center gap-1 px-3 py-1 rounded-full bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200">
                                    <Users className="w-3 h-3" />
                                    {task.assigned_to.map(u => u.full_name || u.email).join(', ')}
                                  </span>
                                )}
                              </div>
                            </div>
              ...
              {modalTasks.length > 0 && (
                <div className="bg-gray-50 dark:bg-gray-900 px-6 py-4 flex justify-between items-center">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Showing {modalTasks.length} {modalTasks.length === 1 ? 'task' : 'tasks'}
                  </p>
                  <Link
                    to="/tasks"
